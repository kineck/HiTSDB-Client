package com.aliyun.hitsdb.client.retry;
...
public class TestHiTSDBClientBatchPutRetryCallback {
    ...
    public void init() throws HttpClientInitException {
        BatchPutCallback pcb = new BatchPutCallback() {
            ...
            public void failed(String address, List<Point> points, Exception ex) {
                ...
                System.err.println(
                        "已处理" + count + "个点." + "---" +
                                "业务回调出错！" + points.size() + " error!");
//                ex.printStackTrace();
            }

            @Override
            public void response(String address, List<Point> points, Result output) {
                int count = num.addAndGet(points.size());
